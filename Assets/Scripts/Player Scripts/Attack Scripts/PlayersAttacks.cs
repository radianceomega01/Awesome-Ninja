...
public class PlayersAttacks : MonoBehaviour {
	...
	public Sprite skillOne_Avail;
	public Sprite skillTwo_Avail;
	public Sprite skillThree_Avail;

	public Sprite skill_Unavail;
	...
	void HandleButtonPresses() {
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (s1_NotUsed)
            {
                s1_NotUsed = false;
                anim.SetBool(ANIMATION_SKILL_1, true);
				skillOne_Btn.image.sprite = skill_Unavail;
        ...
        if (Input.GetKeyDown(KeyCode.Q))
        {
            if (s2_NotUsed)
            {
                s2_NotUsed = false;
                anim.SetBool(ANIMATION_SKILL_2, true);
				skillTwo_Btn.image.sprite = skill_Unavail;
        ...
        if (Input.GetKeyDown(KeyCode.R))
        {
            if (s3_NotUsed)
            {
                s3_NotUsed = false;
                anim.SetBool(ANIMATION_SKILL_3, true);
				skillThree_Btn.image.sprite = skill_Unavail;
	...
	IEnumerator ResetSkills(int skill) {
		...
		case 1:
			s1_NotUsed = true;
			skillOne_Btn.image.sprite = skillOne_Avail;
			break;

		case 2:
			s2_NotUsed = true;
				skillTwo_Btn.image.sprite = skillTwo_Avail;
				break;

		case 3:
			s3_NotUsed = true;
				skillThree_Btn.image.sprite = skillThree_Avail;
				break;
		}
	}
